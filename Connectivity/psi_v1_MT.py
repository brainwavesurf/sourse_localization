...
#compute PSI between all vertices in V1 label and MT label
...
for subject in SUBJECTS:
    ...
    for vert_num_v1 in range(len(stcs_slow_v1[0].vertices[1])):
        ...
        v1_label = mne.read_label(datapath + 'Results_Alpha_and_Gamma/' + subject + '/' + subject + '_V1_rh.label')
        v1_label.vertices = stcs_slow_v1[0].vertices[1][vert_num_v1]
            
        seed_ts_slow_v1 = mne.extract_label_time_course(stcs_slow_v1, v1_label, src, mode='mean_flip',
                                                            verbose='error')
        ...
        for vert_num_mt in range(len(stcs_slow_mt[0].vertices[1])):
            ...
            # Now, we generate seed time series from each vertex in the left V1
            mt_label = mne.read_label(datapath + 'Results_Alpha_and_Gamma/' + subject + '/' + subject + '_MT_rh.label')
            mt_label.vertices = stcs_slow_mt[0].vertices[1][vert_num_mt]
                
            seed_ts_slow_mt = mne.extract_label_time_course(stcs_slow_mt, mt_label, src, mode='mean_flip',
                                                                verbose='error')
...
time = stop-start
np.save(savepath + 'psi/' + 'all_v1_mt_vertices_rh', all_v1_mt)

all_vertices = np.load(savepath + 'psi/' + 'all_v1_mt_vertices_rh.npy')
plt.scatter(np.arange(42), all_vertices.mean(1))
plt.title('All vertices time courses')
plt.plot(np.arange(42), np.zeros(42), 'r--')
plt.ylim(-0.3,0.3)
plt.ylabel('PSI')
plt.xlabel('Subjects')
plt.savefig(savepath + 'psi/' + 'all_v1_mt_vertices')
