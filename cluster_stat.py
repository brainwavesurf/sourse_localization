...
for subject in SUBJECTS:
    ...
    n_vertices_sample, n_times = stc_fsaverage_slow.data.shape
    n_subjects = len(SUBJECTS)
    
    X = randn(n_vertices_sample, n_times, n_subjects, 2)
    X[:, :, :, 0] += stc_fsaverage_slow.data[:, :, np.newaxis]
    X[:, :, :, 1] += stc_fsaverage_fast.data[:, :, np.newaxis]
...
tstep = stc_fsaverage_fast.tstep  
